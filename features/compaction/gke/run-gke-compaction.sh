...
GKE_CLUSTER_NAME="${GKE_CLUSTER_NAME:-compaction-e2e}"
...
GKE_COMPACTION_MODE="${GKE_COMPACTION_MODE:-single-pass}"
DESCHEDULER_LOGS=""
...
apply_workloads() {
  ...
  run_cmd kubectl_ctx rollout status deploy/busy-a -n compaction-e2e --timeout=180s
  run_cmd kubectl_ctx rollout status deploy/busy-b -n compaction-e2e --timeout=180s
  run_cmd kubectl_ctx wait --for=condition=Ready pod/candidate-0 -n compaction-e2e --timeout=180s
  kubectl_ctx get pod -n compaction-e2e candidate-0 -o wide
  wait_for_pod_on_node compaction-e2e candidate-0 "${LIGHT_NODE}"
}
...
wait_for_rebound_blockers() {
  ...
  run_cmd kubectl_ctx rollout status deploy/rebound-busy-a -n compaction-e2e --timeout=180s
  run_cmd kubectl_ctx rollout status deploy/rebound-busy-b -n compaction-e2e --timeout=180s
}
...
verify_move() {
  local new_node
  new_node="$(wait_for_pod_node_change compaction-e2e candidate-0 "${LIGHT_NODE}")"
  ...
  kubectl_ctx get pods -n compaction-e2e -o wide
...
verify_organic_rebound() {
  local initial_uid="$1"
  local rebound_uid
  rebound_uid="$(wait_for_pod_uid_change compaction-e2e candidate-0 "${initial_uid}")"

  run_cmd kubectl_ctx wait --for=condition=Ready pod/candidate-0 -n compaction-e2e --timeout=180s
  wait_for_pod_on_node compaction-e2e candidate-0 "${LIGHT_NODE}"

  if [[ "${DESCHEDULER_LOGS}" != *'Evicted pod'* || "${DESCHEDULER_LOGS}" != *'compaction-e2e/candidate-0'* ]]; then
...
case "${GKE_COMPACTION_MODE}" in
  ...
  organic-rebound)
    ...
    CANDIDATE_UID="$(kubectl_ctx get pod candidate-0 -n compaction-e2e -o jsonpath='{.metadata.uid}')"
    enable_rebound_blockers
    open_busy_nodes
    wait_for_rebound_blockers
    run_descheduler_once
    ...
    ;;
  *)
    echo "Unknown GKE_COMPACTION_MODE: ${GKE_COMPACTION_MODE}" >&2
