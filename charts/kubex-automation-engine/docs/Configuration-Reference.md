...
Every Kubex custom resource exposes these status fields:

| Field | Meaning |
| --- | --- |
| `status.outcome` | `Success` or `Failure`. An omitted value means reconciliation is still in progress. |
| `status.reason` | Human-readable detail about the current result. Proposal sync forwards it as the proposal message. |
| `status.observedGeneration` | The resource generation that produced the result. Proposal sync waits for this to match `metadata.generation`. |

Proposal-managed resources map to `IN_PROGRESS` while `outcome` is missing or stale, `APPLIED` for current `Success`, and `FAILED` for current `Failure`. Decode, validation, ownership, and Kubernetes apply errors report `FAILED` immediately.

### CR outcome rules

Controllers evaluate the resource configuration, not the presence of work at the moment of reconciliation. A valid policy with no matching workloads, namespaces, nodes, or current action can still report `Success`.

| Resource kinds | Success | Failure |
| --- | --- | --- |
| `AutomationStrategy`, `ClusterAutomationStrategy` | The admitted generation is available to consumers | Creation or validation errors are reported by the proposal API when no CR exists |
| `StaticPolicy`, `ClusterStaticPolicy`, `ProactivePolicy`, `ClusterProactivePolicy`, `GpuReactivePolicy`, `ClusterGpuReactivePolicy` | Scope and strategy resolve and policy reconciliation completes | Invalid scope or unresolved strategy |
| `PodAffinityPolicy`, `ContainerArgsPolicy` | Scope is valid and policy reconciliation completes | Invalid scope or runtime setup failure |
| `RollbackPolicy`, `ClusterRollbackPolicy` | The generation is accepted and registered | Reconciliation failure |
| `PolicyEvaluation` | The singleton is ready to process pod events | Required setup failure |
| `GlobalConfiguration` | Required reload, rescan, and health setup completes | A required operation fails |
| `ClusterCompactionPolicy` | Validation and scheduler or descheduler setup completes | Invalid configuration or runtime setup failure |
| `GpuConsolidationPolicy` | An evaluation completes, including no matching nodes or no candidate | Listing, planning, or eviction fails |
| `ObjectPatch`, `ClusterObjectPatch` | The target reaches the requested state | Terminal error, drift, or missing target after application |
