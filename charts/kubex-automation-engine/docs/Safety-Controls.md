...
- Normal workload events with reason `ActionFilteredByAutoscaler` when HPA or VPA filters suppress resize actions
...
| `vpa-resource-managed` | pod action filter | `spec.safetyChecks.enableVpaFilter`, `spec.safetyChecks.blockResizeOnVpaControlledResources` | Two-stage filter: (1) when `enableVpaFilter=true`, removes actions for resources with active VPA recommendations (requires updateMode enabled and RecommendationProvided status); (2) when `blockResizeOnVpaControlledResources=true`, and only after stage 1 has run, also removes actions for resources declared in VPA resourcePolicy regardless of VPA state. Stage 2 is more defensive and blocks even when VPA is Off or has no recommendations. See VPA Filter Behavior section in Automation Strategies docs for details. | target list only; stage 1 produces no metadata; stage 2 adds `source=vpa`, `vpaName`, `updateMode`, and `ownership=resourcePolicy` metadata | Stage 1: `VPA manages <resource>`. Stage 2: `VPA <name> (updateMode=<mode>) owns <resource>.<request|limit>` |
