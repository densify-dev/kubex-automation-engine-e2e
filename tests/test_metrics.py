...
class TestMetrics:
    ...
    def _get_metrics(self, kube_context: str, controller_namespace: str) -> str:
        ...
        if not svc_name:
            pytest.skip("Metrics service not found")
    ...
    def test_reconcile_metrics_present(self, kube_context, controller_namespace):
        metrics = self._get_metrics(kube_context, controller_namespace)
        assert "controller_runtime_reconcile_total" in metrics, (
            "Expected controller_runtime_reconcile_total metric"
        )

    def test_globalconfiguration_reconcile_counted(self, kube_context, controller_namespace):
        metrics = self._get_metrics(kube_context, controller_namespace)
